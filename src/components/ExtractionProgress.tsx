...
export function describeUnavailable(unavailable: Unavailable | null): string {
  if (!unavailable) return "LLM API를 사용할 수 없어 일시 중단했습니다.";
  const tail = ` (${unavailable.detail})`;
  if (unavailable.kind === "auth") return `LLM API 키가 거부되었습니다. API key를 확인해 주세요.${tail}`;
  ...
  if (/EACCES|EPERM/.test(unavailable.detail)) {
    return `서버의 외부 네트워크 연결이 차단되어 있습니다.${tail}`;
  }
  ...
  if (/Timeout|Abort/.test(unavailable.detail)) {
    return `LLM 응답이 늦어 일시 중단했습니다.${tail}`;
  }
  if (/invalid_json/.test(unavailable.detail)) return `LLM 응답 형식 오류로 일시 중단했습니다.${tail}`;
  if (unavailable.kind === "server") return `LLM 서버 오류로 일시 중단했습니다.${tail}`;
  return `LLM API에 연결하지 못했습니다.${tail}`;
}
...
const PAUSE_TEXT: Record<PauseReason, string> = {
  "rate-limit": "LLM 요청 한도에 도달해 일시 중단했습니다.",
  "daily-limit": "LLM 일일 한도를 모두 사용했습니다. 한도가 초기화된 뒤 이어서 추출할 수 있습니다.",
  budget: "요청 상한 또는 크레딧 한도에 도달해 중단했습니다.",
    ...
    <section className="rounded-lg bg-navy-950 p-4 text-sm text-white sm:px-[18px]" aria-label="일정 추출 진행">
      <div className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
        ...
        {running && (
          <span className="text-xs text-mist-300" aria-live="polite">
            ...
            {llmEnabled && sinceUpdate !== null && sinceUpdate >= 5 ? " · LLM 응답 대기 중" : ""}
          </span>
        )}
        <span className="ml-auto font-display text-sm font-bold text-ark-300">{percent}%</span>
      </div>
      <div
        className="mt-2.5 h-2 overflow-hidden rounded-sm bg-white/15"
        role="progressbar"
        aria-valuemin={0}
        aria-valuemax={total}
        aria-valuenow={done}
      ...
      {run.important > 0 && (
        <p className="mt-2.5 text-xs text-amber-300">
          ★ 중요 후보 <strong className="font-display text-white">{run.important.toLocaleString()}</strong>건{" "}
      ...
      {waiting && run.paused === "unavailable" ? (
        <p className="mt-3 rounded border border-amber-300/40 bg-amber-300/10 p-2 text-xs text-amber-100" aria-live="polite">
          {describeUnavailable(run.unavailable)} <strong className="font-display">{resumeIn}초</strong> 뒤 다시 시도합니다.
        </p>
      ) : waiting ? (
        <p className="mt-3 rounded border border-ark-300/40 bg-ark-300/10 p-2 text-xs text-mist-100" aria-live="polite">
          LLM 요청 한도에 도달했습니다. <strong className="font-display text-white">{resumeIn}초</strong> 뒤 자동으로 이어 갑니다.
        </p>
      ) : (
        !running &&
        run.paused && (
      ...
      <div className="mt-3 flex flex-wrap justify-end gap-2 text-xs">
        {running && (
          <button
            onClick={onStop}
            disabled={stopping}
            className="rounded-[3px] border border-ark-300/50 px-2.5 py-1 text-ark-300 hover:bg-white/5 disabled:opacity-50"
          >
        ...
        {!running && (overall.pending > 0 || overall.failed > 0) && (
          <button onClick={onResume} className="rounded-[3px] border border-ark-300/50 px-2.5 py-1 text-ark-300 hover:bg-white/5">
            {waiting ? "지금 재시도" : overall.failed > 0 ? "다시 시도" : "이어서 추출"}
          </button>
        )}
        {waiting && (
