...
export default async function GoogleBulkSendPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="rounded-lg border border-slate-200 bg-white p-3 text-sm tabular-nums">
        {scope === "important" ? "중요 일정 중 " : ""}보낼 수 있음 <strong>{plan.sendable.length.toLocaleString()}</strong> · 이미 생성됨{" "}
        {plan.created.length.toLocaleString()} · 보낼 수 없음 {plan.blocked.length.toLocaleString()}
      ...
      {connection.state !== "connected" ? (
        ...
        <GoogleBulkSend
          key={`${scope}:${items.map((item) => item.id).join(",")}`}
          items={items}
          ...
          <ul className="mt-2 divide-y divide-slate-100">
            {plan.created.map(({ event, editedSince, syncedAt }) => (
              <li key={event.id} className="flex flex-wrap items-baseline gap-x-2 py-1.5">
                ...
                {editedSince && (
                  <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs text-amber-800">이후 로컬 수정은 Google에 반영 안 됨</span>
                )}
              </li>
            ))}
          </ul>
