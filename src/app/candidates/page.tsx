    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Approve하면 BYPP 안의 캘린더에 일정이 추가되고, Ignore하거나 되돌리면 캘린더에서 빠집니다. 승인만으로는 Google 캘린더에 아무것도 전송되지
          않습니다.
        </p>
      </div>

      <SourcePicker
        key={`picker-${values.source}`}
        status={active}
        selected={scope.kind === "group" ? scope.group.key : ""}
        options={view.groups.map((group) => ({ key: group.key, title: group.title, total: group.total, pending: group.counts.PENDING }))}
        totalCandidates={totalCandidates}
        searchFields={{ ...search, ...importanceField }}
        allHref={hrefWith({})}
      />
      {scope.kind === "unknown" && (
      ...
      {view.sections.map((section) => {
        ...
        return (
          <section key={group.key} aria-label={group.title} className="space-y-3">
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 border-b border-slate-300 pb-1">
              <h2 className="min-w-0 break-words text-base font-semibold">{group.title}</h2>
              <span className="text-sm tabular-nums text-slate-500">
                {shown.length < total ? `${shown.length.toLocaleString()} / ${total.toLocaleString()}건 표시` : `${total.toLocaleString()}건`}
              </span>
              {section.important > 0 && !important && <span className="text-xs text-amber-700">★ 중요 {section.important.toLocaleString()}건</span>}
              {scope.kind !== "group" && (
                <Link href={hrefWith({ source: group.key })} className="ml-auto text-sm text-slate-600 underline">
                  이 채팅방만 보기
                </Link>
              )}
            </div>
            {group.filenames.length > 0 && (
              <p className="break-words text-xs text-slate-500">
                {group.filenames[0]}
                {group.filenames.length > 1 ? ` 외 ${group.filenames.length - 1}개 (인원수만 다른 같은 채팅방의 파일)` : ""}
              </p>
            )}
            {shown.map((candidate) => (
